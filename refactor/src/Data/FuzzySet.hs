...
getWithMinScore minScore set@FuzzySet{ gramSizeLower = lower, gramSizeUpper = upper, .. } value =
    case HashMap.lookup key exactSet of
        ...
        Nothing -> 
            getMatches key minScore set <$> sizes
                & find (not . null)
                & fromMaybe []
  where
...
addToSet set@FuzzySet{ gramSizeLower = lower, gramSizeUpper = upper, .. } value
  ...
  where
    ...
    addSize gramSize FuzzySet{..} =
        ...
        in
        FuzzySet{ items = items & HashMap.insert gramSize (itemVector `snoc` item)
    ...
    updateExactSet value FuzzySet{..} =
        FuzzySet{ exactSet = exactSet & HashMap.insert key value
                , .. }
